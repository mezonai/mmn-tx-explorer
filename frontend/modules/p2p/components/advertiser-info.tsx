...
export const AdvertiserInfo = ({ advertiser, isClanOffer }: AdvertiserInfoProps) => {
  return (
    <div className="flex items-center gap-3">
      ...
      <div>
        <div className="flex items-center gap-1 font-bold">
          {isClanOffer ? (
            <span className="text-white dark:text-white">{advertiser.username}</span>
          ) : (
          ...
          {isClanOffer && (
            <span className="ml-1 rounded bg-brand-primary px-1.5 text-[10px] text-white">
              CLAN MEMBER
            </span>
          )}
        </div>
