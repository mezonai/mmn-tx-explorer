...
func RunOrchestrator(cmd *cobra.Command, args []string) {
	...
	// Run PostgreSQL migrations for main/staging/orchestrator
	migrationsPath := os.Getenv("MIGRATIONS_PATH")
	log.Info().Msg("Running PostgreSQL migrations...")
	if err := migration.RunPostgresMigrations(migrationsPath); err != nil {
		log.Fatal().Err(err).Msg("Failed to run PostgreSQL migrations")
	}
