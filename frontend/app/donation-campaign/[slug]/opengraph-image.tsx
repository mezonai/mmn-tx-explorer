...
export default async function Image({ params }: { params: Promise<{ slug: string }> }) {
  try {
    const fontDataRegularPromise = fetch(
      new URL(
        '../../../assets/font/UcCO3FwrK3iLTeHuS_fvQtMwCp50KnMw2boKoduKmMEVuLyfAZ9hjp-Ek-_EeA.woff',
        import.meta.url
      )
    ...
    const fontDataBoldPromise = fetch(
      new URL(
        '../../../assets/font/UcCO3FwrK3iLTeHuS_fvQtMwCp50KnMw2boKoduKmMEVuFuYAZ9hjp-Ek-_EeA.woff',
        import.meta.url
      )
    ).then((res) => res.arrayBuffer());

    const [fontDataRegular, fontDataBold] = await Promise.all([
      fontDataRegularPromise.catch((e) => {
        console.error('OG Font Regular load error:', e);
        return null;
      }),
      fontDataBoldPromise.catch((e) => {
        console.error('OG Font Bold load error:', e);
        return null;
      }),
    ]);
    ...
    if (hasWallet) {
      ...
      const qrDataStr = encodeURIComponent(JSON.stringify(qrObject));
      qrCodeData = `https://api.qrserver.com/v1/create-qr-code/?size=400x400&data=${qrDataStr}&color=ffffff&bgcolor=1e293b&margin=1`;
    }
            ...
            <div tw="flex flex-col">
              ...
              {endDate && (
                <div tw="flex items-center">
                  <div tw="h-1.5 w-1.5 rounded-full mr-3 bg-[#00f5ff]" />
