...
export const ChatSidebar = ({ sellerId }: ChatSidebarProps) => {
  ...
  return (
    <div className="flex h-full min-h-0 w-full flex-col border-l border-gray-200 bg-white transition-colors duration-200 md:w-87.5 lg:w-125 dark:border-gray-800 dark:bg-[#0B0E11]">
      {/* Header */}
      ...
      >
        {/* Security Awareness & Chat Guidance */}
        ...
        {messages.map((msg, idx) => {
          ...
          const showDateDivider = !prevMsg || !isSameDay(msg.create_time, prevMsg.create_time);
          const isFirstInGroup = !prevMsg || prevMsg.sender_id !== msg.sender_id || showDateDivider;
          const isLastInGroup =
            !nextMsg ||
            nextMsg.sender_id !== msg.sender_id ||
            (nextMsg && !isSameDay(msg.create_time, nextMsg.create_time));
            ...
            <div key={msg.message_id}>
              {showDateDivider && (
                <div className="my-6 flex items-center justify-center">
                  <span className="rounded-full border border-gray-200 bg-gray-100 px-3 py-1 text-[10px] font-bold text-gray-500 uppercase dark:border-gray-800 dark:bg-gray-900">
                    {new Date(Number(msg.create_time)).toLocaleDateString([], {
                      month: 'short',
                      day: 'numeric',
                      year: 'numeric',
                    })}
              ...
              >
                ...
                <div className={cn('flex w-full items-end gap-2', isMe ? 'flex-row-reverse' : 'flex-row')}>
                  ...
                  <div
                    className={cn(
                      'w-fit max-w-full rounded-2xl px-3 py-2 text-sm shadow-sm transition-colors',
                      'overflow-wrap-anywhere break-all whitespace-pre-wrap',
                      isMe
                ...
                {isLastInGroup && (
                  <div className={cn('mt-1 px-1 text-[9px] font-bold text-gray-400 uppercase', !isMe && 'ml-10')}>
                    {formatChatTime(msg.update_time)}
                  </div>
                )}
              </div>
            </div>
      ...
      <div className="shrink-0 border-t border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-[#0B0E11]">
