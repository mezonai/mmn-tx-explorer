            ...
            <DialogContent onOpenAutoFocus={(e) => e.preventDefault()} className="sm:max-w-106.25">
                ...
                <div className="mt-4 space-y-3 rounded-lg border border-border bg-muted/30 p-4">
                    <div className="flex items-center justify-between border-b border-border pb-3">
                        <span className="text-xs text-muted-foreground">Amount to Buy</span>
                        <span className="text-lg font-bold text-foreground">
                            {formatCurrency(amountToBuy)} <span className="text-sm text-muted-foreground">{APP_CONFIG.CHAIN_SYMBOL}</span>
                        </span>
                    </div>
