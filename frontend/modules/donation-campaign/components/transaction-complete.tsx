...
export const TransactionComplete = (props: TransactionCompleteProps) => {
  ...
  const description = finalType === TransactionType.Withdraw
  ...
  return (
    <div className="relative flex flex-col items-center space-y-5 text-center"> 
      ...
      <div className="bg-brand-primary/10 border-brand-primary/40 borde mt-1 w-full rounded-xl p-5 focus:ring-0 focus:outline-none"> 
        <div className="flex flex-col items-center space-y-4"> 
          <div className="text-center"> 
            <p className="text-xs uppercase">Amount {finalType === TransactionType.Withdraw ? 'Withdrawn' : 'Donated'}</p>
          ...
          {finalType === TransactionType.Donation && (
            <div className="w-full border-t"></div>
          )}

          {txHash && (
            <div className="w-full space-y-2 text-left">
              <p className="text-xs uppercase text-center">Transaction Hash</p>
      ...
      {onClose && (
        <Button
          onClick={onClose}
          className="bg-brand-primary hover:bg-brand-primary/85 w-full rounded-xl py-3 text-sm font-semibold text-white shadow-lg transition flex items-center justify-center group"
        >
          Close
        </Button>
      )}
